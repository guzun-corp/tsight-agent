...
async fn test_process_next_acquire_failure() {
    ...
    assert_eq!(
        error_msg,
        "Failed to acquire next job from server: Failed to acquire job: 500 Internal Server Error",
...
async fn test_process_next_task_not_found() {
    ...
    let error_msg = result.unwrap_err().to_string();
    assert_eq!(
        error_msg, "Failed to acquire next job from server: No jobs available",
        "Error message doesn't match expected content"
    );
    acquire_mock.assert();
}
...
async fn test_process_next_execution_failure() {
    ...
    let error_message = "Query execution error for query: Query execution error: HTTP status client error (404 Not Found) for url (http://localhost:8123/)";
